...
function getActiveSubtitleTrack() {
  if (!videoElement.value) return;
  return Array.from(videoElement.value.textTracks).find(
...
function updateTimeline() {
  ...
  videoPlayerContainer.value.style.setProperty(
    "--progress",
    percent.toString(),
  );
...
function toggleFullscreen() {
  ...
  if (fullscreenState) {
    ...
    if (videoPlayerContainer.value?.webkitRequestFullscreen)
      videoPlayerContainer.value?.webkitRequestFullscreen();
    else if (videoPlayerContainer.value?.requestFullscreen)
      videoPlayerContainer.value?.requestFullscreen();
    else if (videoPlayerContainer.value?.mozRequestFullScreen)
      videoPlayerContainer.value?.mozRequestFullScreen(); // Careful to the capital S
    else if (videoPlayerContainer.value?.msRequestFullscreen)
      videoPlayerContainer.value?.msRequestFullscreen();
    else if (videoPlayerContainer.value?.webkitEnterFullscreen)
      videoPlayerContainer.value?.webkitEnterFullscreen(); // Magic is here for iOS
  } else {
    if (document.exitFullscreen) document.exitFullscreen();
    else if (videoPlayerContainer.value?.webkitCancelFullscreen)
      videoPlayerContainer.value?.webkitCancelFullscreen();
    else if (videoPlayerContainer.value?.mozCancelFullScreen)
      videoPlayerContainer.value?.mozCancelFullScreen(); // Careful
    else if (videoPlayerContainer.value?.msExitFullscreen)
      videoPlayerContainer.value?.msExitFullscreen();
    exitFullscreen();
  }
}
...
function selectPreferredOrAvailableLanguage(langs: Language[]) {
  const preferredLanguage = langs.find(
    (obj) => obj.language == preferredWatchLanguage.value,
  );
...
function setPosterPath() {
  posterSrc.value = APIConnector.getPosterURL(
      ...
      <div class="video-controls-container">
        ...
        <div class="controls">
          <div class="left-controls">
            <div
              class="control-element play-btn"
          ...
          <div class="right-controls">
            <div class="control-element lang-btn popoverInvoker" tabindex="0">
              ...
              <LanguagePop
                ...
                id="language-select"
                :availableLangs="
                  availableLanguages.map((lang) => lang.language)
                "
            ...
            <div class="control-element sub popoverInvoker" tabindex="0">
              ...
              <SubtitlePop
                class="popover"
                id="subtitles-select"
                :languages="subTitleData.map((item) => item.language)"
                @setSubtitle="setSubtitle"
              />
            </div>
            <div
              class="control-element fullscreen-btn"
              @click="toggleFullscreen"
              tabindex="0"
            >
              <img
                src="../assets/control-icons/fullscreen.svg"
                id="fullscreen-icon"
              />
              <img
                src="../assets/control-icons/fullscreen-exit.svg"
